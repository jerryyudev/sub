...
install_x-ui() {
    ...
    # Using custom download link instead of GitHub
    wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://jerrypersonal.pages.dev/x-ui-linux-amd64.tar.gz
    if [[ $? -ne 0 ]]; then
        echo -e "${red}下载 x-ui 失败，请确保你的下载链接有效${plain}"
        exit 1
    fi
